...
const LEGACY_KEY = '20250717-dev/wallet.example.com.json'
const NEW_KEY = '20260305-dev/wallet.example.com.json'
...
function makeNewConfiguration(): Configuration {
  return {
    $walletAddress: '$wallet.example.com',
...
describe('migrateSingle', () => {
  ...
  it('checks new prefix first, then gets legacy data, puts new, DOES NOT delete legacy', async () => {
    ...
    mockGetJson.mockResolvedValueOnce(makeLegacyData())
    ...
    expect(mockGetJson).toHaveBeenCalledWith(LEGACY_KEY)
  ...
  it('produces the correct Configuration shape from legacy data', async () => {
    ...
    try {
      ...
      expect(uploaded).toEqual(makeNewConfiguration())
    } finally {
      vi.useRealTimers()
    }
  })
  ...
  it('preserves banner fields in the uploaded data', async () => {
    ...
    const v1 = makeLegacyData().version1
  ...
  it('migrates all profile versions from a multi-version legacy config', async () => {
    ...
    const multi = {
      version1: { ...legacy.version1, versionName: 'Profile 1' },
      version3: { ...legacy.version3, versionName: 'Profile 3' },
    }
